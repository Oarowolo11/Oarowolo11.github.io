# Olayiwola Arowolo — personal academic website

Live at **https://oarowolo11.github.io**.

Built with the [AcademicPages](https://github.com/academicpages/academicpages.github.io) Jekyll template (a fork of Minimal Mistakes) and hosted on GitHub Pages, which builds the site automatically from the `master` branch.

## Editing

- Homepage bio: `_pages/about.md`
- Sidebar profile & social links: the `author:` block in `_config.yml`
- Publications: one Markdown file per paper in `_publications/`
- Talks: `_talks/` · Teaching: `_teaching/` · Portfolio: `_portfolio/` · Blog posts: `_posts/`
- CV page: `_pages/cv.md` (PDF at `files/Olayiwola_Arowolo_CV.pdf`)

## Local preview (optional)

Requires Ruby 3.2+:

```bash
bundle install
bundle exec jekyll serve -l
```
